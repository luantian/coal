    ...
    <div class="table_div" style="position: relative">
      ...
      <div style="position: absolute;top:65px;left:20px;right:42px;bottom:20px;overflow: auto;padding-right: 0px">
        <el-table :data="tableData" stripe style="width: 100%;">
          ...
          <el-table-column prop="partsLocation" label="部件位置" align="center"></el-table-column>
          <el-table-column prop="specification" label="规格型号" align="center"></el-table-column>
          <el-table-column prop="technicalParam" label="技术参数" align="center"></el-table-column>
...
import addForm from "@/views/DeviceArchivesReport/StrapParts/AddForm";
import editForm from "@/views/DeviceArchivesReport/StrapParts/EditForm";
...
export default {
  ...
  methods: {
    async toQuery() {
      ...
      const {rows, total} = await ArchiveReportModel.selectStrapFileList(this.queryParams);
      this.tableData = rows
    ...
    async deleteData(index, row) {
      this.$confirm('是否确认删除该数据?', '提示', {
        ...
        const {code} = await ArchiveReportModel.deleteStrapFile(row.id)
        if (code === 200) {
          this.$message({
            message: '删除成功',
            type: 'success'
