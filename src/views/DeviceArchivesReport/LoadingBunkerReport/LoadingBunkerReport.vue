<template>
  <div class="loading-bunker-report">
    ...
    <div class="table_div" style="position: relative">
      <div style="display: flex;">
        ...
        <div style="padding-right: 35px">
          <el-button @click="addDialogVisible=true" type="primary">新增参数2</el-button>
        </div>
      </div>
      <div class="main_table">
        <el-table :data="tableData" stripe style="width: 100%;" height="calc(100vh - 400px)">
          ...
          <el-table-column prop="warehouseWidth" label="仓体宽度(m)" align="center"></el-table-column>
          <el-table-column prop="warehouseHeight" label="仓体高度(m)" align="center"></el-table-column>
          <el-table-column prop="warehouseVolume" label="仓体容积(m³)" align="center"></el-table-column>
          <el-table-column prop="materialDensity" label="物料容重(t/m³)" align="center"></el-table-column>
          <el-table-column prop="materialAngle" label="物料堆积角(°)" align="center"></el-table-column>
...
import addForm from "@/views/DeviceArchivesReport/LoadingBunkerReport/AddForm";
import editForm from "@/views/DeviceArchivesReport/LoadingBunkerReport/EditForm";
import { cloneDeep } from 'lodash-es'
export default {
  name: 'LoadingBunkerReport',
  ...
  methods: {
    async toQuery() {
      ...
      const {rows, total} = await ArchiveReportModel.selectLoadingBunkerFilesList(this.queryParams);
      this.tableData = rows
    ...
    async deleteData(index, row) {
      this.$confirm('是否确认删除该数据?', '提示', {
        ...
        const {code} = await ArchiveReportModel.deleteLoadingBunkerFile(row.id)
        if (code === 200) {
          this.$message({
            message: '删除成功',
            type: 'success'
